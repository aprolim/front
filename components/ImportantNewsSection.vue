...
console.log('🔍 [ImportantNewsSection] Iniciando carga de datos...');

// 🔥 CORRECCIÓN: useFetch con manejo de errores y timeout
const { data, pending, error, refresh } = await useFetch(API_BASE_URL, {
  ...
  onRequest({ request, options }) {
    console.log('📡 [ImportantNewsSection] Solicitando:', request);
  },
  onResponse({ request, response, options }) {
    console.log('✅ [ImportantNewsSection] Respuesta recibida:', response.status);
  },
  onResponseError({ request, response, options }) {
    console.error('❌ [ImportantNewsSection] Error en respuesta:', response.status, response.statusText);
    console.error('❌ URL:', request);
  },
  transform: (response) => {
    console.log('🔄 [ImportantNewsSection] Transformando respuesta...');
    if (response?.success && response?.data) {
      console.log('✅ Datos transformados correctamente');
      return response.data;
    }
    if (response?.tabs) {
      console.log('✅ Datos con formato alternativo');
      return response;
    }
...
if (error.value) {
  console.error('❌ [ImportantNewsSection] Error detectado:', error.value);
}

const activeTab = ref('');

const tabs = computed(() => {
  const resultado = data.value?.tabs || [];
  console.log(`📊 [ImportantNewsSection] ${resultado.length} tabs cargados`);
  return resultado;
});
...
const generatePath = (tabId, item) => {
  ...
  try {
    ...
    console.warn('⚠️ Error generando path:', err);
    return `/${tabId}/item`; 
  }
};
...
if (tabs.value.length > 0 && !activeTab.value) {
  ...
  console.log(`📌 [ImportantNewsSection] Tab activo: ${activeTab.value}`);
}
