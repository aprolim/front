      ...
      <div class="management-header">
        <div class="header-left">
          <button 
            ...
            title="Volver a las opciones de gestión"
          >
        ...
        <div class="global-search-wrapper">
          ...
          <div class="global-search-box z-30">
            <input 
              ...
              placeholder="Buscar en todos los documentos (título, número, asunto)..."
            ...
            <table class="resultados-tabla">
              <thead>
                <tr>
                  ...
                  <th @click="ordenarResultadosPor('asunto')" class="sortable">
                    Asunto
                    <span class="sort-icon">{{ getResultadosSortIcon('asunto') }}</span>
                  </th>
                  <th @click="ordenarResultadosPor('gestion')" class="sortable">
                    Gestión
                    <span class="sort-icon">{{ getResultadosSortIcon('gestion') }}</span>
                  </th>
                  <th @click="ordenarResultadosPor('institucion')" class="sortable">
                    Institución
                    <span class="sort-icon">{{ getResultadosSortIcon('institucion') }}</span>
                  </th>
                  <th class="actions-column">Documento</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="item in itemsResultadosPaginados" :key="item.id">
                  ...
                  <td class="resultado-gestion">
                    {{ item.gestion || '-' }}
                  </td>
                  <td class="resultado-institucion">
                    {{ item.institucion_text || '-' }}
                  </td>
        ...
        <div v-if="!busquedaGlobal" class="tabs-content px-1 sm:px-2 md:px-3 lg:px-4 xl:px-5 py-2 sm:py-3 md:py-4 lg:py-5 xl:py-6">
          ...
          <div class="section-header text-center pb-0.5 sm:pb-1 mb-1 sm:mb-2 border-b border-[#E03636] border-opacity-30">
            <h2 class="section-title font-bold text-[#E03636] text-[3.0vw] mb-0 translate-y-[-7vw]">
              {{ areaGestion?.titulo || 'Gestión' }}
            </h2>
            <p class="section-description text-gray-600 text-[.9vw] mx-auto translate-y-[-7vw]">
              {{ areaGestion?.descripcion || 'Documentación y resoluciones de gestión institucional' }}
            </p>
          </div>
          
          <!-- Grid de Tarjetas - Navegación entre los diferentes tipos de gestión -->
          <div class="links-grid grid gap-1 sm:gap-1.5 md:gap-2 xl:gap-4 3xl:gap-6 5xl:gap-9 grid-cols-3">
            <NuxtLink 
              v-for="link in linksGestion" 
              :key="link.id"
              :to="link.path" 
...
// Configuración de endpoints para gestión
const ENDPOINTS = {
  'resoluciones-camarales': {
    ...
    procesarRespuesta: (data) => {
      if (data.data && data.data.data) {
        return data.data.data.map(item => ({ ...item, _categoria: 'resoluciones-camarales' }))
      }
      return []
    },
  ...
  'declaraciones-camarales': {
    url: 'https://apisi.senado.gob.bo/page/declaraciones-camarales',
    titulo: 'Declaraciones Camarales',
    icono: '',
    procesarRespuesta: (data) => {
      if (data.data && data.data.data) {
        return data.data.data.map(item => ({ ...item, _categoria: 'declaraciones-camarales' }))
      }
      return []
    },
    obtenerTotalPaginas: (data) => {
  ...
  'minutas-comunicacion': {
    url: 'https://apisi.senado.gob.bo/page/minutas-comunicacion',
    titulo: 'Minutas de Comunicación',
    icono: '',
    procesarRespuesta: (data) => {
      if (data.data && data.data.data) {
        return data.data.data.map(item => ({ ...item, _categoria: 'minutas-comunicacion' }))
      }
      return []
    },
    obtenerTotalPaginas: (data) => {
...
// Funciones para extraer datos del título para ordenamiento en gestión
...
const tituloSeccion = computed(() => {
  return endpointConfig.value?.titulo || 'Resoluciones Camarales'
})
...
const iconoActivo = computed(() => {
  ...
  const link = linksGestion.value.find(l => {
    return l.path && l.path.includes(`filtro=${filtroActual.value}`)
  })
  return link?.icono || ''
})

// Datos de gestión desde la API de tabs
const areaGestion = computed(() => {
  return tabsData.value?.areas?.gestion || null
})

const linksGestion = computed(() => {
  return tabsData.value?.links?.gestion || []
})
...
const resultadosOrdenados = computed(() => {
  ...
  return [...resultadosGlobal.value].sort((a, b) => {
    if (resultadosOrden.value.columna === 'gestion') {
      const gestionA = a.gestion || 0
      const gestionB = b.gestion || 0
      if (resultadosOrden.value.direccion === 'asc') {
        return gestionA - gestionB
      } else {
        return gestionB - gestionA
      }
    ...
    } else if (resultadosOrden.value.columna === 'asunto') {
      const asuntoA = a.asunto || ''
      const asuntoB = b.asunto || ''
      if (resultadosOrden.value.direccion === 'asc') {
        return asuntoA.localeCompare(asuntoB)
      } else {
        return asuntoB.localeCompare(asuntoA)
      }
    } else if (resultadosOrden.value.columna === 'institucion') {
      const instA = a.institucion_text || ''
      const instB = b.institucion_text || ''
      if (resultadosOrden.value.direccion === 'asc') {
...
const obtenerNombreCategoria = (categoriaKey) => {
  const nombres = {
    'resoluciones-camarales': 'Resoluciones Camarales',
    'declaraciones-camarales': 'Declaraciones Camarales',
    'minutas-comunicacion': 'Minutas de Comunicación'
  }
...
const limpiarBusquedaGlobal = () => {
  ...
  resultadosOrden.value = { columna: 'gestion', direccion: 'desc' }
}

// Función para volver a las opciones de gestión
...
const cargarTabsGestion = async () => {
  ...
  try {
    ...
    if (data?.success && data?.data) {
      ...
      if (tabsData.value?.links?.gestion) {
        tabsData.value.links.gestion.forEach(link => {
...
<style scoped>
/* Estilos - mismos que antes con las adiciones para el buscador global */
