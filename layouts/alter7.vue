<template>
  <div class="min-h-screen">
    ...
    <header class="fixed top-0 left-0 right-0 z-50 bg-transparent">
      <!-- Línea roja superior con color sólido -->
    ...
    >
      ...
      <!-- Menú lateral - COLOR COMPLETAMENTE SÓLIDO Y UNIFORME -->
      <aside 
        :class="[
          'absolute left-0 w-72 bg-[#E03636] text-white shadow-2xl transform transition-transform duration-300 ease-in-out',
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        ]"
        :style="{ top: '56px', height: 'calc(100vh - 56px)' }"
      >
        <!-- Encabezado del sidebar - MISMO COLOR SÓLIDO -->
        <div class="p-5 border-b border-[#E4D294]/30 bg-[#E03636]">
        ...
        <div class="p-4 overflow-y-auto h-[calc(100%-140px)] bg-[#E03636]">
          <!-- Menú de Constitución -->
          <div class="mb-3">
            <button 
              @click="toggleConstitution"
              class="w-full flex items-center justify-between p-3 rounded-lg hover:bg-[#d13030] transition-colors text-left group border border-[#E4D294]/10 bg-[#E03636]"
            >
              <div class="flex items-center gap-3">
            ...
            <div 
              ...
              class="ml-10 mt-2 mb-3 space-y-2"
            >
              <NuxtLink 
                to="/alternative/constitucion/texto-completo" 
                class="block py-2.5 px-4 rounded-lg hover:bg-[#d13030] transition-colors text-sm text-[#E4D294] hover:text-white group bg-[#E03636] border border-[#E4D294]/10"
                @click="sidebarOpen = false"
              >
          ...
          <NuxtLink 
            ...
            class="flex items-center gap-3 p-3 rounded-lg hover:bg-[#d13030] transition-colors mb-2 group border border-[#E4D294]/10 bg-[#E03636]"
            @click="sidebarOpen = false"
          >
            <div class="w-8 h-8 bg-[#E4D294]/20 rounded-lg flex items-center justify-center group-hover:bg-[#E4D294]/30">
              <svg class="w-4 h-4 text-[#E4D294]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <NuxtLink 
            ...
            class="flex items-center gap-3 p-3 rounded-lg hover:bg-[#d13030] transition-colors group border border-[#E4D294]/10 bg-[#E03636]"
            @click="sidebarOpen = false"
          >
            <div class="w-8 h-8 bg-[#E4D294]/20 rounded-lg flex items-center justify-center group-hover:bg-[#E4D294]/30">
              <svg class="w-4 h-4 text-[#E4D294]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Pie del sidebar - MISMO COLOR SÓLIDO -->
        <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-[#E4D294]/30 bg-[#E03636]">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-[#E4D294] text-sm font-medium">Senado Nacional</p>
              <p class="text-[#E4D294]/70 text-xs">Versión 2.0</p>
            </div>
            <button 
              @click="sidebarOpen = false"
              class="px-3 py-1.5 bg-[#d13030] hover:bg-[#c52e2e] text-white rounded-lg transition-colors text-sm font-medium border border-[#E4D294]/30"
            >
              ✕ Cerrar
            </button>
          </div>
...
const legislatorsOpen = ref(false)   // Legisladores NO desplegado
...
/* COLOR PRINCIPAL SÓLIDO Y UNIFORME */
.bg-\[\#E03636\] {
  background-color: #E03636 !important;
}

/* Color para hover y estados activos */
.bg-\[\#d13030\] {
  background-color: #d13030 !important;
}
...
.border-\[\#E4D294\]\/10 {
  border-color: rgba(228, 210, 148, 0.1) !important;
}

.border-\[\#E4D294\]\/30 {
  border-color: rgba(228, 210, 148, 0.3) !important;
}
...
/* Hover effects */
.hover\:bg-\[\#d13030\]:hover {
  background-color: #d13030 !important;
}

.hover\:bg-\[\#c52e2e\]:hover {
  background-color: #c52e2e !important;
}

/* Asegurar que todo el menú tenga el mismo color sólido */
aside, 
aside > div,
aside .bg-\[\#E03636\] {
  background-color: #E03636 !important;
