...
<template>
  <div class="min-h-screen py-12" style="background-color: #d9d9d9;">
    <div class="container mx-auto px-4">
      ...
      <!-- NOTICIAS AGRUPADAS POR FILAS DE 4 -->
      <div v-else-if="noticias.length > 0">
        <div 
          v-for="(fila, filaIndex) in noticiasPorFilas" 
          :key="filaIndex"
          class="rounded-xl overflow-hidden mb-6"
          :class="filaIndex % 2 === 0 ? 'bg-white' : 'bg-[rgba(224,54,54,0.08)]'"
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-6">
            <div 
              v-for="noticia in fila" 
              :key="noticia.id"
              class="group cursor-pointer rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-white"
      ...
      <div v-if="totalPaginas > 1 && !loading" class="flex justify-center gap-2 mt-12">
        <button @click="paginaActual--" :disabled="paginaActual === 1" class="px-4 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition">Anterior</button>
        <div class="flex gap-1">
          <button v-for="p in paginasMostradas" :key="p" @click="paginaActual = p" :class="['w-10 h-10 rounded-lg transition', paginaActual === p ? 'bg-[#E03636] text-white' : 'bg-white border border-gray-300 hover:bg-gray-100']">{{ p }}</button>
        </div>
        <button @click="paginaActual++" :disabled="paginaActual === totalPaginas" class="px-4 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition">Siguiente</button>
      </div>
    </div>
  </div>
</template>
...
const noticiasPorFilas = computed(() => {
  const filas = []
  for (let i = 0; i < noticias.value.length; i += 4) {
    filas.push(noticias.value.slice(i, i + 4))
  }
  return filas
})
