    ...
    <main class="main-content">
      ...
      <div class="hemicycle-container">
        <div class="hemicycle-header">
          <h3>🏛️ Distribución en el Hemiciclo del Senado</h3>
          <div class="hemicycle-info">
            <div class="info-item">
              <span>Fila 1: 14 asientos</span>
              <span>Fila 2: 22 asientos</span>
            </div>
            <div class="info-item">
              <span>Pasillo central divide la cámara</span>
            </div>
          </div>
        </div>

        <!-- Mesa Directiva (Norte) -->
        ...
        <!-- Hemiciclo SVG con distribución correcta -->
        <div class="hemicycle-svg-container">
          ...
          >
            <!-- Fondo del hemiciclo -->
            <defs>
              <radialGradient id="chamberBg" cx="50%" cy="30%" r="70%">
            ...
            <!-- Fondo semicircular -->
            <path 
              d="M 100,400 Q 500,150 900,400 L 900,600 Q 500,350 100,600 Z" 
              fill="url(#chamberBg)" 
              stroke="#cbd5e1" 
              stroke-width="2"
            />
            ...
            <!-- Pasillo central -->
            <rect x="495" y="250" width="10" height="200" fill="#94a3b8" opacity="0.3"/>
            <text x="500" y="470" text-anchor="middle" fill="#64748b" font-size="12">
              Pasillo Central
            </text>

            <!-- Líneas guía -->
            <path 
              d="M 150,380 Q 500,200 850,380" 
              fill="none" 
              stroke="#e2e8f0" 
              stroke-width="1" 
            ...
            <!-- Fila 1 (14 asientos, más cerca de la presidencia) -->
            <g class="row-1">
            ...
            <text x="100" y="650" fill="#4b5563" font-size="14" font-weight="600">
              Izquierda
            </text>
            <text x="900" y="650" fill="#4b5563" font-size="14" font-weight="600" text-anchor="end">
              Derecha
            </text>
            
            <!-- Fila labels -->
            <text x="50" y="320" fill="#64748b" font-size="12" text-anchor="end">
              Fila 1 (14)
            </text>
            <text x="50" y="470" fill="#64748b" font-size="12" text-anchor="end">
              Fila 2 (22)
            </text>
          </svg>
          ...
          <div class="senator-header">
            <div class="photo-section">
              <div class="photo-container">
                <div class="senator-photo">{{ selectedSenator.initials }}</div>
...
const senatorsData = ref([
  ...
  // Fila 2 - 22 asientos (más alejada de la presidencia)
...
// Configuración de distribución en media luna con pasillo central
const generateSeatPositions = () => {
  const centerX = 500
  const row1Y = 320  // Fila más cercana a la presidencia
  const row2Y = 470  // Fila más alejada
  
  // Configuración para media luna
  const row1Left = []   // Fila 1, lado izquierdo (asientos 1-7)
  const row1Right = []  // Fila 1, lado derecho (asientos 8-14)
  const row2Left = []   // Fila 2, lado izquierdo (asientos 15-25)
  const row2Right = []  // Fila 2, lado derecho (asientos 26-36)
  
  // Procesar cada senador y asignar posición
  senatorsData.value.forEach(senator => {
    let x, y
    const seatNum = senator.seatNumber
    
    if (senator.row === 1) {
      y = row1Y
      // Distribución en media luna para fila 1 (14 asientos)
      if (seatNum <= 7) {
        // Lado izquierdo del pasillo (izquierda de la cámara)
        const position = seatNum - 1
        const angle = Math.PI * 0.6 * (position / 6) - Math.PI * 0.3
        x = centerX - 200 + Math.cos(angle) * 180
      } else {
        // Lado derecho del pasillo (derecha de la cámara)
        const position = seatNum - 8
        const angle = Math.PI * 0.6 * (position / 6) + Math.PI * 0.7
        x = centerX + 200 + Math.cos(angle) * 180
      }
    } else {
      y = row2Y
      // Distribución en media luna para fila 2 (22 asientos)
      if (seatNum <= 25) {
        // Lado izquierdo del pasillo
        const position = seatNum - 15
        const angle = Math.PI * 0.7 * (position / 10) - Math.PI * 0.35
        x = centerX - 250 + Math.cos(angle) * 220
      } else {
        // Lado derecho del pasillo
        const position = seatNum - 26
        const angle = Math.PI * 0.7 * (position / 10) + Math.PI * 0.65
        x = centerX + 250 + Math.cos(angle) * 220
      }
    }
    
    // Actualizar senador con coordenadas
    senator.x = x
    senator.y = y
    
    // Agrupar para referencia
    if (senator.row === 1) {
      if (seatNum <= 7) row1Left.push(senator)
      else row1Right.push(senator)
    } else {
      if (seatNum <= 25) row2Left.push(senator)
      else row2Right.push(senator)
    }
  })
  
  // Ordenar por número de asiento
  const row1Seats = [...row1Left, ...row1Right].sort((a, b) => a.seatNumber - b.seatNumber)
  const row2Seats = [...row2Left, ...row2Right].sort((a, b) => a.seatNumber - b.seatNumber)
  
  return { row1Seats, row2Seats }
}
...
const selectSenator = (senator) => {
  ...
  hoveredSeat.value = null // Limpiar hover
}
...
// Actualizar posición del tooltip cuando hay hover
onMounted(() => {
  // Configurar evento para tooltip estático
  document.addEventListener('mousemove', (e) => {
    if (hoveredSeat.value && !selectedSenator.value) {
      // Posicionar tooltip cerca del mouse pero fijo
      const container = document.querySelector('.hemicycle-svg-container')
      if (container) {
        const rect = container.getBoundingClientRect()
        tooltipStyle.left = `${e.clientX - rect.left + 15}px`
        tooltipStyle.top = `${e.clientY - rect.top + 15}px`
      }
    }
  })
})
